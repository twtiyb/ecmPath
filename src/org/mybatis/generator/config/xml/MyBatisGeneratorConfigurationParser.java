...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseContext(Configuration configuration, Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            } else if ("iserviceGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseIserviceGenerator(context, childNode);
    ...
    private void parseIserviceGenerator(Context context, Node node) {
        IServiceGeneratorConfiguration iserviceGeneratorConfiguration = new IServiceGeneratorConfiguration();

        context.setIserviceGeneratorConfiguration(iserviceGeneratorConfiguration);

        Properties attributes = parseAttributes(node);
        String targetPackage = attributes.getProperty("targetPackage"); //$NON-NLS-1$
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$

        iserviceGeneratorConfiguration.setTargetPackage(targetPackage);
        iserviceGeneratorConfiguration.setTargetProject(targetProject);
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseProperty(iserviceGeneratorConfiguration, childNode);
            }
        }
    }
    private void parseServiceGenerator(Context context, Node node) {
        ServiceGeneratorConfiguration serviceGeneratorConfiguration = new ServiceGeneratorConfiguration();

        context.setServiceGeneratorConfiguration(serviceGeneratorConfiguration);
